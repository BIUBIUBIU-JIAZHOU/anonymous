...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if constraint_utils is not None and constraint_utils['constraint_decoding']:
            ...
            for i in range(decoder_input_ids.shape[0]):
                lm_logits_mask = torch.full_like(lm_logits[i, :, :], -math.inf)
                # lm_logits_mask = torch.full_like(lm_logits[i, :, :], -math.inf).unsqueeze(1)
                cur_decoder_id = decoder_input_ids[i]
                constraint_utils['decode_ids'][i].append(cur_decoder_id)
                # cur_decoder_ids = constraint_utils['decode_ids'][i]
                cur_decoder_ids = torch.cat([decode_id for decode_id in constraint_utils['decode_ids'][i]], dim=0)
                cur_input_ids = input_ids[i]
                # 返回在input_ids中，元素值是'['的位置
                left_brace_index = (cur_decoder_ids == to_id['['][0]).nonzero()
                # 返回在input_ids中，元素值是']'的位置
                right_brace_index = (cur_decoder_ids == to_id[']'][0]).nonzero()
                ...
                cur_id = cur_decoder_ids[-1]

                # 如果当前token是'['，那么下一个token集合就是'O，A，S，C，SS'
                if cur_id in to_id['[']:
                    # return force_tokens['special_tokens']
                    for special_token in force_tokens['special_tokens']:
                        lm_logits_mask[0, special_token] = 0
                    # for j in range(lm_logits.size(2)):
                    #     lm_logits[i, :, j] += lm_logits_mask[0, j]
                    lm_logits[i, :, :] += lm_logits_mask
                    # for special_token in force_tokens['special_tokens']:
                    #     print(lm_logits[i, :, special_token])
                    continue
                # 如果当前token是'O，A，S，C，EP'，那么下一个token集合就是']'
                elif cur_id in to_id['AT'] + to_id['OT'] + to_id['EP'] + to_id['SP'] + to_id['AC']:
                    # return to_id[']']
                    lm_logits_mask[0, to_id[']'][0]] = 0
                    # for j in range(lm_logits.size(2)):
                    #     lm_logits[i, :, j] += lm_logits_mask[0, j]
                    lm_logits[i, :, :] += lm_logits_mask
                    continue
                # 如果当前token是'SS'，那么下一个token集合就是'EP'
                elif cur_id in to_id['SS']:
                    # return to_id['EP']
                    lm_logits_mask[0, to_id['EP'][0]] = 0
                    # for j in range(lm_logits.size(2)):
                    #     lm_logits[i, :, j] += lm_logits_mask[0, j]
                    lm_logits[i, :, :] += lm_logits_mask
                    continue
                ...
                # get cur_term
                # 如果不存在'['，那么下一个token集合是'[，</s>'
                if last_left_brace_pos == -1:
                    # return to_id['['] + [1]  # start of sentence: [
                    lm_logits_mask[0, to_id['['][0]] = 0
                    lm_logits_mask[0, 1] = 0
                    # for j in range(lm_logits.size(2)):
                    #     lm_logits[i, :, j] += lm_logits_mask[0, j]
                    # print(lm_logits[i, :, :].shape)
                    lm_logits[i, :, :] += lm_logits_mask
                    # print(lm_logits[i, :, to_id['['][0]])
                    # print(lm_logits[i, :, 1])
                    continue
                #
                    ...
                    cur_term = cur_decoder_ids[last_left_brace_pos + 1]

                ret = []
                # 如果当前token是'S'，那么下一个token集合就是'_great, _ok, _bad'
                if cur_term in to_id['SP']:  # SP
                    # ret = force_tokens['sentiment_tokens'][task]
                    ret = [1465, 2841, 7163]  # positive, negative, neutral
                    # ret = [2748, 14261, 5854]  # pos, neg, neu
                    # ret = [207, 1282, 1825]  # good, bad, ok
                    # ret = [248, 1282, 1825]  # great, bad, ok

                # 如果当前token是'A'，那么下一个token集合就是source_ids
                elif cur_term in to_id['AT']:  # AT
                    force_list = cur_input_ids.tolist()
                    # if input_ids[i][1] != 8323:
                    #     force_list.extend(to_id['it'] + [1])
                    # force_list.extend(to_id['it'] + [1])
                    if constraint_utils["is_labels"][i][0] != 0:
                        force_list.extend(to_id['null'])
                    ret = force_list
                # 如果当前token是'SS'，那么下一个token集合就是'</s>, ], _'
                elif cur_term in to_id['SS']:
                    ret = [3] + to_id[']'] + [1]
                # 如果当前token是'C'，那么下一个token集合就是'cate_tokens'
                # _rest:880, _laptop:4544, 14:2534, 15:1808, 16:2938

                elif cur_term in to_id['AC']:  # AC
                    # top10_input_ids = input_ids[i][:10].tolist()
                    # if 880 in top10_input_ids:
                    #     data_type_index = top10_input_ids.index(880)
                    #     data_type = to_id['880'] + to_id[str(top10_input_ids[data_type_index + 1])]
                    # if 4544 in top10_input_ids:
                    #     data_type_index = top10_input_ids.index(4544)
                    #     data_type = to_id['4544'] + to_id[str(top10_input_ids[data_type_index + 1])]
                    ret = list(set(force_tokens['cate_tokens'][constraint_utils["dataset"][i]]))
                # 如果当前token是'O'，那么下一个token集合就是source_ids
                elif cur_term in to_id['OT']:  # OT
                    force_list = cur_input_ids.tolist()
                    # if input_ids[i][2] == 509:
                    #     force_list.extend(to_id['null'])
                    if constraint_utils["is_labels"][i][1] != 0:
                        force_list.extend(to_id['null'])
                    ret = force_list
                else:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        if "encoder_outputs" not in model_kwargs:
            ...
            irrelevant_prefix = ["decoder_", "cross_attn", "use_cache", "next_ids", "constraint_decoding", "is_labels", "dataset"]
            encoder_kwargs = {
                argument: value
            ...
            model_kwargs['constraint_utils'] = {'markers': [[] for _ in range(inputs_tensor.shape[0])],
                                                ...
                                                'is_labels': model_kwargs['is_labels'],
